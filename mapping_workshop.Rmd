...
- Analyze and plot raster data to address a research question  
- Apply different mapping functions to plot raster data  
...
**Question 1:** How does predicted lilac leaf out vary across CONUS? Any ideas what may explain these patterns? 
...
The map is more informative if we add state boundaries. We'll get these data from the [`USAboundaries`](https://docs.ropensci.org/USAboundaries/) package. Before doing this, however, we need to use `st_use_s2(FALSE)` because `sf` assumes the world is round, and uses great circles for straight lines. This will cause issues for cropping boundaries to the same extent as our raster data.
```{r}
sf_use_s2(FALSE)
# US states feature
states <- us_states()
...
```{r}
states_c2 <- filter(states, !state_abbr %in% c("AK", "HI", "PR"))
plot(states_c2[1])
```

Finally, use `geom_sf()` to add an `sf` object to a plot - i.e., the states feature. Notice what happens when we do NOT enter `NA` for fill for our states feature. Why does this happen?
```{r, fig.height = 5, fig.width=9.5}
...
Finally, use `geom_sf()` to add an `sf` object to a plot - i.e., the states feature. Notice what happens when we do NOT enter `NA` for fill for our states feature. 
```{r, fig.height = 5, fig.width=9.5}
# Add an 'sf' object to them map
ggplot() + 
  geom_raster(data = lilacModel_df, aes(x = x, y = y, fill = leaf_out_month)) +
  geom_sf(data = states_c, fill = NA) +
  geom_sf_text(data = states_c, aes(label = state_abbr), size = 2) +
  scale_fill_manual(name = "Predicted First\nLeaf Out", label = labs, values = pal) +
  ggtitle("Month of Predicted First Leaf Out in Lilac in 2018") +
  my_theme
```
...
**Question 4:** How and why does the geometry of the lilac observations differ from the states feature?
...
**Question 5:** What does your map tell you about predicted vs. observed first leaf out in lilac? Can you think of any better ways to compare the two datasets on a map? 
...
We can also add county boundaries, which can be downloaded via `us_counties()`. Feel free to convert the geometry to `MULTILINESTRING`.
```{r}
...
zoom_modVobs <- conus_modVobs +
  geom_sf(data = counties, color = "gray30", size = 0.25) + # Add counties
  geom_sf(data = lilacObs_sf, aes(color = leaf_out_month), size = 3, show.legend = FALSE) +
  coord_sf(xlim = c(-90.5542, -82.3047), 
           ylim = c(41.6311, 47.5739),
...
lilacObs_extr <- extract(lilacModel_r, lilacObs_sf) %>%
  rename("leaf_out_month_pred" = "leaf_out_month") %>%
  as.data.frame() %>%
  bind_cols(lilacObs_sf) %>%
  

ggplot(lilacObs_extr, aes(x = leaf_out_month_pred)) +
  geom_bar() +
  ggtitle("Predicted leaf out month")
```
```{r}
ggplot(lilacObs_extr, aes(x = leaf_out_month)) +
  geom_bar() +
  ggtitle("Observed leaf out month")
```
### **Wrap-up**
